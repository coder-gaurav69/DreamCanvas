...
const NavBar = () => {
  ...
  const { loginStatus, setLoginStatus, profileImage } =
  ...
  return (
    ...
    >
          ...
          >
            {profileImage ? (
              <img
                src={profileImage}
                alt="Profile"
                className="profile-img rounded-full w-[35px] h-[35px]"
              />
            ) : (
              <FaUserCircle
                className={`${
      ...
      {toggleMenu && (
        <div
          className={`flex flex-col absolute top-[60px] left-0 w-[100%] p-[8px_8px_12px_8px] md:hidden text-white bg-black`}
        >
          <Link
            to={"/"}
